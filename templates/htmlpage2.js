...
//this version being used to replace main image with canvas & edit.tf renderer.
...
console.log("update_altlist");
  ...
  d.getFullYear() + "T" + ("0" + d.getHours()).slice(-2) + ":" + ("0" + d.getMinutes()).slice(-2);
			...
			idcount++;
			canvas = cli.getElementsByTagName('canvas')[0];
			canvas.setAttribute('id','frm' + idcount);
			var div = upTo(canvas, 'div');
			div.setAttribute('id','canvasfrm' + idcount);
...
///			cli.querySelector('.i_pagename').innerHTML = alt.frame_id + alt.subframe_id;
			...
			// initialise editor (cannot do prior to being added to DOM)
			var editor = new Editor();	// teletext-editor.js

			editors[datestring] = editor;
			editors[datestring].canvasid = 'frm' + idcount;


			// only if selected item
			if (alt.canonbase == canonbase) {

				// Set up the screen and render it.
				editors[datestring].init_state();
				font = editors[datestring].font;
	//			editors[datestring].render(0, 0, 40, 25, 0);

				editors[datestring].wipe(0);
				editors[datestring].load(alt.tf);
			//
			}
//console.log (altlist);
		}
...
console.log("blsuccess");
	var el = upTo(ele, 'li');
	var datestring = el.dataset.date;
...
// and load subframes.
		if (ele.classList.contains('mainimage')) {
			// Set up the screen and render it.
			if (font != null) {
			 	editors[datestring].font = font;
			}
			editors[datestring].init_state();
	//		editors[datestring].render(0, 0, 40, 25, 0);

			editors[datestring].wipe(0);
			editors[datestring].load(el.dataset.tf);	// loads and renders image. Slow!
		...
		ele.classList.add('b-loaded');	// force flag to say done.
	}
...
console.log('imgswap called on ', e);
	...
	li.querySelectorAll('.tflink').forEach(function(f){
						f.href = f.href.replace('/\#.*/i', '#' + e.dataset.tf);
				});

	// these probably don't really need doing, as will be set on initial page creation
	// and this checking any particular url by (e.g.) facebook will have correct values
	// already in them.
